...
SELECT 
    ...
    name,
    CASE 
        WHEN type = 'supplier' THEN 'supplier'
...
WHERE type IN ('supplier', 'company')
    AND status = 'active'  -- Only migrate active tenants
...
SELECT 
    ...
    u.updated_at
FROM users_backup u
INNER JOIN tenants_backup t ON u.tenant_id = t.id
WHERE u.tenant_id IS NOT NULL
    AND u.status = 'active'  -- Only migrate active users
